...
const { audioEffectsEnabled } = useHomeAssistant();
...
onMounted(() => {
  // For now we initialize immediately in dev mode to avoid permission issues
...
// Watch for audio effects enabled state from Home Assistant
watch(audioEffectsEnabled, (enabled) => {
  if (enabled && !audioPermissionGranted.value) {
    initAudio();
  } else if (!enabled && audioContext.value) {
    // Stop audio processing when disabled
    if (animationFrameId.value) {
      cancelAnimationFrame(animationFrameId.value);
    }
