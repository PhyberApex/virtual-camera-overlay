<template>
  <div
    ...
    class="fixed top-8 left-8 flex justify-start z-50"
  >
    <div 
    ...
    >
      ...
      <!-- Heart rate value -->
      <div class="mb-2">
        <span class="text-5xl font-extrabold text-white drop-shadow-md block leading-none">{{ heartRate }}</span>
        <span class="text-lg text-white/90 font-semibold drop-shadow-sm">BPM</span>
      </div>
      
      <!-- Heart rate zone indicator -->
      <div class="mt-1">
        <span class="text-sm font-semibold text-white/80 uppercase tracking-wider drop-shadow-sm">{{ getHeartRateZone() }}</span>
      </div>
    </div>
  </div>
</template>
